...
def transition(trans, stack, buffer, arcs):
    ...
    if action == SH:
        ...
        arcs.append((stack[1], stack[0], label))
        stack.pop(0)
    elif action == RA:
        arcs.append((stack[0], stack[1], label))
        stack.pop(1)
...
def oracle(stack, buffer, heads, labels, arcs):
    ...
    top = stack[0]
    next_on_stack = stack[1]

    if heads[next_on_stack] == top:
        if all_dependents_found(next_on_stack, buffer, heads):
            return (LA, labels[next_on_stack])
    
    if heads[top] == next_on_stack:
        if all_dependents_found(top, buffer, heads):
            return (RA, labels[top])
